<x-indexLay>
    <div class="w-full px-4 sm:px-8 lg:px-12 xl:px-16 py-12 relative z-20">
        ...
        <div class="flex flex-col md:flex-row gap-8 lg:gap-16">
            ...
            <div class="flex-none w-full md:w-1/3 lg:w-[400px]">
                <div class="relative rounded-3xl overflow-hidden shadow-2xl shadow-brand-bg/50">
                    <img src="{{ isset($top['poster_path']) && $top['poster_path'] ? 'https://image.tmdb.org/t/p/original/'.$top['poster_path'] : 'https://via.placeholder.com/500x750' }}" alt="TV Show Poster" class="w-full h-auto object-cover">
                </div>
            </div>
            ...
            <div class="flex-grow flex flex-col justify-center">
                ...
                <div class="flex flex-wrap items-center gap-3 mb-8">
                    ...
                    <div class="flex items-center space-x-1.5 bg-yellow-500/10 border border-yellow-500/20 px-3 py-1.5 rounded-lg text-yellow-500 font-bold text-sm">
                        ...
                        <span>{{$top['vote_average'] ?? 'N/A'}}/10</span>
                    </div>

                    <!-- Release / Status Badge -->
                    @if(isset($top['first_air_date']) || isset($top['release_date']))
                    <div class="bg-gray-800 border border-gray-600 px-3 py-1.5 rounded-lg text-gray-300 font-bold text-sm">
                        {{ \Carbon\Carbon::parse($top['first_air_date'] ?? $top['release_date'])->format('M d, Y') }}
                    </div>
                    @endif
                </div>
                ...
                <div x-data="{ isOpen: false }">
                    <div class="flex flex-wrap gap-4 mb-12">
                        @if (isset($top['videos']['results']) && count($top['videos']['results']) > 0)
                        ...
                        <div class="w-full max-w-5xl bg-brand-bg rounded-2xl overflow-hidden shadow-2xl relative" @click.away="isOpen = false">
                            ...
                            <div class="relative pt-[56.25%] w-full">
                                <template x-if="isOpen">
                                    <iframe class="absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/{{ $top['videos']['results'][0]['key'] ?? '' }}?autoplay=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
                                </template>
                            </div>
                        </div>
                    </div>
        ...
        <!-- CAST SECTION -->
        @if(isset($top['credits']['cast']) && count($top['credits']['cast']) > 0)
        <div class="mt-24 pt-12 border-t border-gray-800">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                @foreach (array_slice($top['credits']['cast'], 0, 10) as $cast)
                    <div class="group">
        ...
        <!-- TV SHOW PICTURES / BACKDROPS -->
        @if(isset($top['images']['backdrops']) && count($top['images']['backdrops']) > 0)
        <div class="mt-24 pt-12 border-t border-gray-800">
            <h2 class="text-3xl font-bold text-white mb-8 border-l-4 border-brand pl-4">Gallery</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach (array_slice($top['images']['backdrops'], 0, 6) as $image)
